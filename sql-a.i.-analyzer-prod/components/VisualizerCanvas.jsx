...
import { 
  ...
  GitMerge, ListFilter, ArrowRightCircle, Plus, Trash2, Move,
  Table as TableIcon, Heading, Layers, Code2, Scissors, Save, Shield,
  ChevronRight, ChevronDown, Link as LinkIcon
} from 'lucide-react';

// --- Node Components ---

const TableNodeComponent = ({ node, style, isSelected, onMouseDown, onDelete }) => (
  <div 
...
const OperationNodeComponent = ({ node, style, isSelected, onMouseDown, onDelete }) => {
  ...
  return (
    <div 
      className={`absolute px-3 py-2 rounded-md border shadow-lg flex items-center justify-between gap-2 transition-all duration-200 cursor-grab active:cursor-grabbing z-20 
...
const ConnectionLine = ({ edge, start, end, onClick, selected }) => {
...
export const VisualizerCanvas = ({ visualFlow, isLoading, onFlowChange, mockDb }) => {
  ...
  const emitChange = useCallback((updatedNodes, updatedEdges) => {
    if (onFlowChange) {
  ...
  const handleMouseUp = (e, targetNodeId) => {
    // Finish Drawing Edge
    if (drawingEdge) {
  ...
  const handleSidebarDragStart = (e, type, payload = '') => {
  ...
  return (
    <div className="w-full h-full bg-[#0B1120] relative overflow-hidden group flex" onKeyDown={handleKeyDown} tabIndex={0}>
                                ...
                                {expandedTables.has(tableName) && (
                                    <div className="bg-slate-950/30 p-2 space-y-1 border-t border-slate-800">
                                        {mockDb[tableName].schema.map(col => (
                                            <div key={col.name} className="flex items-center justify-between px-2 py-1 rounded hover:bg-slate-800/50 text-[10px]">
                                                 <div className="flex items-center gap-2">
        ...
        <div className="flex-1 relative h-full">
            ...
            <div className="absolute top-4 left-4 z-40 bg-slate-900/80 backdrop-blur border border-slate-700 rounded px-3 py-2 text-xs text-slate-400 pointer-events-none">
                Shift + Drag between nodes to link them
            </div>
            ...
            >
                ...
                >
                    ...
                    {nodes.map(node => {
                        ...
                        const nodeWithDrop = (
                            <div onMouseUp={(e) => { e.stopPropagation(); handleMouseUp(e, node.id); }}>
                                {node.type === 'TABLE' ? <TableNodeComponent {...props} /> : <OperationNodeComponent {...props} />}
                            </div>
                        );

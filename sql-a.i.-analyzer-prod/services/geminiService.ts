...
const analyzeSqlClient = async (query: string, apiKey: string): Promise<AnalysisResult> => {
...
const analyzeSqlBackend = async (query: string, apiKey: string): Promise<AnalysisResult> => {
    try {
        ...
        console.error(e);
        throw new Error(`FastAPI Connection Failed: ${e instanceof Error ? e.message : 'Unknown error'}. Ensure main.py is running on port 8000.`);
    }
};

export const analyzeSql = async (query: string, apiKey: string, useBackend: boolean = false): Promise<AnalysisResult> => {

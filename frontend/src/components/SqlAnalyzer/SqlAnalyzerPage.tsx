...
import { Cpu, Code, Play, Layout, AlertCircle, FunctionSquare } from 'lucide-react';
import { analyzeSql } from './services/geminiService';
import { generateSqlFromFlow } from './services/sqlGenerator';
import { AnalysisResult, VisualFlow } from './types';
...
const SqlAnalyzerPage: React.FC = () => {
  const [query, setQuery] = useState(DEFAULT_QUERY);
  const [queryType, setQueryType] = useState<'QUERY' | 'PROCEDURE' | 'FUNCTION'>('QUERY');
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [data, setData] = useState<AnalysisResult | null>(null);
  const [visualFlow, setVisualFlow] = useState<VisualFlow | undefined>(undefined);
  ...
  const handleAnalyze = async () => {
    ...
    setError(null);
    try {
      const result = await analyzeSql(query, queryType);
      setData(result);
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  const handleFlowChange = (newFlow: VisualFlow) => {
    ...
    <div className="h-full w-full bg-slate-950 text-slate-200 font-sans overflow-hidden flex flex-col">
      ...
      <header className="h-14 border-b border-slate-800 bg-slate-900/50 backdrop-blur flex items-center justify-between px-6 shrink-0 z-10">
        ...
        <div className="text-xs text-slate-500 font-mono">
          Configuration loaded from public/config.yaml
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[400px] border-r border-slate-800 bg-slate-900/30 flex flex-col shrink-0 z-10">
          <div className="flex-1 flex flex-col p-4 gap-4 overflow-hidden">
            {/* Input Section */}
            <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl overflow-hidden flex flex-col shrink-0 min-h-[150px] basis-[40%] resize-y">
              <div className="px-4 py-3 border-b border-slate-700/50 bg-slate-900/30 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Code className="w-4 h-4 text-blue-400" />
                  <h3 className="text-sm font-semibold text-slate-200">Input</h3>
                </div>
                {/* Type Toggle */}
                <div className="flex bg-slate-950 rounded p-0.5 border border-slate-700 text-[10px] font-bold">
                  <button
                    onClick={() => setQueryType('QUERY')}
                    className={`px-2 py-0.5 rounded ${queryType === 'QUERY' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}
                  >
                    QUERY
                  </button>
                  <button
                    onClick={() => setQueryType('PROCEDURE')}
                    className={`px-2 py-0.5 rounded ${queryType === 'PROCEDURE' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}
                  >
                    SP
                  </button>
                  <button
                    onClick={() => setQueryType('FUNCTION')}
                    className={`px-2 py-0.5 rounded ${queryType === 'FUNCTION' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}
                  >
                    FUNC
                  </button>
                </div>
              ...
              <textarea
                ...
                placeholder={queryType === 'QUERY' ? "SELECT * FROM orders..." : "CREATE PROCEDURE..."}
                spellCheck={false}
              />
            </div>
            ...
            {data && (
              <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl overflow-hidden flex flex-col flex-1 min-h-[0]">
